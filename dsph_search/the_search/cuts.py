...
def proper_motion_test(dwarf, radius=0.5, cut=None, print_to_stdout=False, test_area = 12, test_percentage = 0.3, num_maxima = 10):
...
def angular_density_test(dwarf, radii=[1, 0.1], print_to_stdout=False):
    """Test for candidacy based on change in object density with changing angular size."""
    # test parameters
    density_tolerance = 1.2
    ...
    for radius in radii:
        ...
        table = table[[5 >= np.hypot(pmra, pmdec) for pmra, pmdec in zip(table['pmra'].data, table['pmdec'].data)]]
        densities.append(len(table)/(radius**2))

    if densities[-1]/densities[0]>1.1:
        if print_to_stdout:
